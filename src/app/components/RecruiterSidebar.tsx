...
import {
  LayoutDashboard, Briefcase, Users, BrainCircuit,
  ClipboardList, Video, BarChart2, Settings,
  Headphones, LogOut,
} from "lucide-react";

const NAV = [
  { label: "Dashboard",    icon: LayoutDashboard, path: "/recruiter" },
  { label: "Job Postings", icon: Briefcase,        path: "/recruiter/jobs" },
  { label: "Applicants",   icon: Users,            path: "/recruiter/applicants" },
  { label: "AI Screening", icon: BrainCircuit,     path: "/recruiter/ai-screening" },
  { label: "Tests",        icon: ClipboardList,    path: "/recruiter/tests" },
  { label: "Interviews",   icon: Video,            path: "/recruiter/interviews" },
  { label: "Reports",      icon: BarChart2,        path: "/recruiter/reports" },
  { label: "Settings",     icon: Settings,         path: "/recruiter/settings" },
  { label: "Help & Support",icon: Headphones,      path: "/recruiter/help" },
];
...
export function RecruiterSidebar() {
  ...
  const handleLogout = () => sessionStorage.removeItem("aira_auth");
        ...
        <div className="relative flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              ...
              <span className="text-white/50 text-[10px] tracking-widest uppercase">Recruiter Panel</span>
            </div>
          </div>
        </div>
      </div>
            ...
            return (
              ...
              >
                ...
                <span className={`text-sm transition-colors duration-200 ${
                  active ? "font-semibold text-[#6750a4]" : "font-normal text-gray-700"
                }`}>
                  {label}
                </span>
                {label === "AI Screening" && (
                  <span className="ml-auto size-2 rounded-full bg-[#17CF97] animate-pulse" />
                )}
              </Link>
            );
          })}
